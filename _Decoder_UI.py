...
class Tools:
    ...
    def make_words_tab(self:'Encoding class', name:str) -> None:
        ...
        self.words = Tools.make_input(self, 7, 'Wordlist:', 'WordsEng', ttk.Combobox, trace=False,
                                      config={'width':12, 'state':'readonly',
                                              'values':('WordsEng', 'WordsGer', 'OUT_Annagram', 'OUT_Regex')})
    ...
    def make_input(self:'Encoding class', column:int, label_text:str, default_value:str,
                   input_object:'tk/ttk input', trace=True, config={}, grid={'sticky':'W'}) -> object:
        ...
        if input_object == ttk.Checkbutton:
            obj.input = input_object(self.tab, variable=obj.value, **config)
        else:
            obj.input = input_object(self.tab, textvariable=obj.value, **config)
        obj.input.grid(column=column, row=2, **grid)
        return obj

    def make_reverse(self:'Encoding class', column:int) -> None:
    ...
    def switch(self:'Encoding class object') -> None:
...
class Morse():
    def __init__(self):
        ...
        self.letter = Tools.make_input(self, 6, 'letter sep', ' ', ttk.Entry, config={'width':5})
        self.word = Tools.make_input(self, 8, 'word sep', '/', ttk.Entry, config={'width':5})
...
# Athbash to translate letters (find better/shorter name)
#   choose table
#      def zxy...cba (= atbash)
#      workman keyboard layout
#      dovrak keyboard layout
#      ?...
#   switch
# analyser
#   tells which encoding it might be
#   box [text, list, grid] (all input is one, each line is one,
